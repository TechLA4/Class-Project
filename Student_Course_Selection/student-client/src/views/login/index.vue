<template id="bg" :style="{backgroundImage:'url('+url+')'}" style="background-size: 100% 100%">
...
export default {
  data() {
    return {
      url: 'https://techla-img.oss-cn-hangzhou.aliyuncs.com/CODE/WEB/DSC06450.jpg',
      ruleForm: {
        id: null,
        password: null,
        type: 'student',
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios
            .get("http://localhost:8081/info/getCurrentTerm")
            .then(function (resp) {
              sessionStorage.setItem("currentTerm", resp.data);
            });

          axios
            .get("http://localhost:8081/info/getForbidCourseSelection")
            .then(function (resp) {
          ...
          if (
            ...
            console.log(form);
            axios
              .post("http://localhost:8081/teacher/login", form)
              .then(function (resp) {
                ...
                if (check === true) {
                  axios
                    .get(
                      "http://localhost:8081/teacher/findById/" +
                        that.ruleForm.id
                    )
                    .then(function (resp) {
            ...
            axios
              .post("http://localhost:8081/student/login", form)
              .then(function (resp) {
