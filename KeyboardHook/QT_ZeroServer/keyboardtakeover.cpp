...
KeyboardTakeOver::KeyboardTakeOver(QWidget *parent) : QWidget(parent)
{
    // 初始化窗口
...
int KeyboardTakeOver::startKeyboardTakeOverServer(QString userName)
{
    // 设置窗口标题
...
void KeyboardTakeOver::newConnection(QTcpSocket *s)
{
    // 新增客户
...
void KeyboardTakeOver::processBuffer()
{
    // 将数据打印到文本框
...
void KeyboardTakeOver::resizeEvent(QResizeEvent *)
{
    mEdit->setGeometry(0,0,width(), height());
}

void KeyboardTakeOver::closeEvent(QCloseEvent *)
{
    // 删除窗口
    deleteLater();
}
