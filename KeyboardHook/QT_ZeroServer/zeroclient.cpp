...
void ZeroClient::sendKeyboardStop(int port)
{
    QString data;
    data.append(CmdKeyboardStop+CmdSplit);
    data.append("PORT"+CmdSplit+QString::number(port));
    data.append(CmdEnd);
    mSock->write(data.toLocal8Bit());
}

void ZeroClient::sendKeyboardTakeOver(QString &text)
{
    QString data;
    data.append(CmdKeyboardTakeOver+CmdSplit);
    data.append("TEXT"+CmdSplit+text);
    data.append(CmdEnd);
    qDebug() << "Data to be sent:" << data;
    mSock->write(data.toLocal8Bit());
}
